...
@dataclass
class PBSPlatformExperimentOperations(FilePlatformExperimentOperations):
    ...
    def platform_cancel(self, experiment_id: str, force: bool = True) -> None:
        ...
        if force or experiment.status == EntityStatus.RUNNING:
            logger.debug(f"cancel pbs job for experiment: {experiment_id}...")
    ...
    def post_run_item(self, experiment: Experiment, dry_run: bool = False, **kwargs):
        ...
        # Not show job details if it is a dry run!
        if dry_run:
            return

        job_ids = self.platform.get_job_id(experiment.id, ItemType.EXPERIMENT)
        if job_ids is None:
            logger.debug(f"PBS job for experiment: {experiment.id} is not available!")
            user_logger.info("PBS Job Ids: None")
        else:
            job_ids = [f'{" ".ljust(3)}{id}' for id in job_ids]
            user_logger.info(f"PBS Job Ids ({len(job_ids)}):")
            user_logger.info('\n'.join(job_ids))
